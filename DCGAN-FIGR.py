...
#         print('classes,class_to_idx:',len(classes),len(class_index))
...
data_dir = 'data/FIGR-8/Data'       # this path depends on your computer
dset = datasets.ImageFolder(data_dir, transform)
train_loader = torch.utils.data.DataLoader(dset, batch_size=128, shuffle=True)


# image_datasets = {x: customData(img_path=data_dir,
#                                     txt_path=('data/FIGR-8/data.txt'),
...
if not os.path.isdir('Runs/DCGAN_FIGR_Results/Random_results'):
    os.mkdir('Runs/DCGAN_FIGR_Results/Random_results')
if not os.path.isdir('Runs/DCGAN_FIGR_Results/Fixed_results'):
    os.mkdir('Runs/DCGAN_FIGR_Results/Fixed_results')
...
for epoch in range(train_epoch):
    ...
    for x_, _ in train_loader:
        ...
        # [128, 3, 64, 64]
        print('discriminator input',np.shape(x_))
        D_result = D(x_).squeeze()
        # [128]
        print('discriminator output',np.shape(D_result))
        ...
        G_result = G(z_)
        # [128, 3, 64, 64]
        print('generator output',np.shape(G_result))
    ...
    p = 'Runs/DCGAN_FIGR_Results/Random_results/' + str(epoch + 1) + '.png'
    fixed_p = 'Runs/DCGAN_FIGR_Results/Fixed_results/' + str(epoch + 1) + '.png'
...
for e in range(train_epoch):
    img_name = 'Runs/DCGAN_FIGR_Results/Fixed_results/' + str(e + 1) + '.png'
