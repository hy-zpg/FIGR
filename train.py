...
1. initialization:
   * env: FIGR8
   * GAN: network initilaization
     * choose models: resnet or dcgan for meta-g, meta-d, g, d
     * optimization: adam
     * targets: d, g
   * checkpoint: load models g, d, episode
2. training:
   ## meta learning
       * reset_meta_model: meta-g, meta-d load trained models checkpoint
       * meta_training_loop:
         * sampling data: supporting data and task
         * inner loop: GAN, (real images, fake images), training generator 
         * Updating both generator and dicriminator
    ## batch learning
       * reset_meta_model: meta-g, meta-d load trained models checkpoint
       * validation_run:
         * sampling data: batch data and tasks
         * inner loop: GAN, (real images, fake images), training generator 
         * generating new images, generated images stored into Validation_generated file, also added generation_loss and discrimination_loss
       * checkpoint_model: saved model
'''
class FIGR:
    ...
    def inner_loop(self, real_batch):
        ...
        # self.writer.add_graph(self.meta_d, (training_batch, ))
        # self.writer.add_graph(self.meta_g, (torch.tensor(np.random.normal(size=(self.batch_size, self.z_shape))), dtype=torch.float, device=device), )
    ...
    def validation_run(self):
        ...
        print('wgan-gp generated image shape',np.shape(img))
    ...
    def meta_training_loop(self):
        ...
        #<class 'torch.Tensor'>
        # [4, 1, 32, 32]
        print('input data',np.shape(data))
        data = normalize_data(data)
        real_batch = data.to(device)
    ...
    def initialize_gan(self):
        ...
        self.meta_d=nn.DataParallel(self.meta_d,device_ids=[0,1])
        self.meta_g=nn.DataParallel(self.meta_g,device_ids=[0,1])
