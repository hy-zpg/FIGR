...
# root = 'data/FIGR-8/Data'
# save_root = 'data/FIGR-8/resize_data'
# resize_size = 64
# if not os.path.isdir(save_root):
#     os.mkdir(save_root)
# if not os.path.isdir(save_root + 'figr'):
#     os.mkdir(save_root + 'figr')
# img_list = os.listdir(root)
# for i in range(len(img_list)):
#     img = plt.imread(root + img_list[i])
#     img = imresize(img, (resize_size, resize_size))
#     plt.imsave(fname=save_root + 'figr/' + img_list[i], arr=img)
#     if (i % 1000) == 0:
...
def FIGR_DCGAN():
    def __init__(self, args):
        ...
        self.img_size = 64
        self.isCrop = False
        self.data_dir = 'data/FIGR-8/Data'
        self.writer = SummaryWriter('Runs/' + 'DCGAN_FIGR')
        self.initialize_gan()
    ...
    def data_preprocess(self):
        ...
        dset = datasets.ImageFolder(self.data_dir, transform)
        train_loader = torch.utils.data.DataLoader(dset, batch_size=self.batch_size, shuffle=True)
        return train_loader
    def network(self):
        self.G = generator(128)
        self.D = discriminator(128)
        ...
        # Adam optimizer
        self.G_optimizer = optim.Adam(G.parameters(), lr=lr, betas=(0.5, 0.999))
        self.D_optimizer = optim.Adam(D.parameters(), lr=lr, betas=(0.5, 0.999))
    ...
    def show_result(self,num_epoch):
        ...
        self.G.eval()
        if isFix:
            test_images = self.G(fixed_z_)
        else:
            test_images = self.G(z_)
        self.G.train()
        size_figure_grid = 5
        ...
        self.writer.add_image('result', fig, num_epoch)
    ...
    def show_train_hist(self, hist):
        ...
        plt.grid(True)
        plt.tight_layout()
        self.writer.add_image('hist_result', plt, num_epoch)
    ...
    def training(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            for x_, _ in self.data_preprocess():
        ...
        self.writer.add_scalar('generator_param.pkl',self.G.state_dict())
        self.writer.add_scalar('discriminator_param.pkl',self.D.state_dict())
...
# results save folder
if not os.path.isdir('FIGR_DCGAN_results'):
    os.mkdir('FIGR_DCGAN_results')
if not os.path.isdir('FIGR_DCGAN_results/Random_results'):
    os.mkdir('FIGR_DCGAN_results/Random_results')
if not os.path.isdir('FIGR_DCGAN_results/Fixed_results'):
    os.mkdir('FIGR_DCGAN_results/Fixed_results')
